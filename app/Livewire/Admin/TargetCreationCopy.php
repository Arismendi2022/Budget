<?php
	...
	class TargetCreationCopy extends Component
	{
		...
		private function resetTargetForm():void{
			...
			$this->reset(['amount','currencyAmount','currencyAmountWeekly','monthlySavingsAmount','selectedText','selectedTextCustom','selectedDay','selectedDayOfWeek','selectedDayText']);
		}
		...
		public function saveTarget():void{
			...
			// Calcular el monto mensual solo cuando la frecuencia es anual (yearly)
		...
		public function calculateMonthlySavings():float{
			$currentDate      = Carbon::now()->startOfMonth(); // 01/05/2025
			$endDate          = Carbon::parse($this->selectedDate)->endOfMonth(); // 31/12/2025
			$monthsDifference = $currentDate->diffInMonths($endDate); // Sin +1
			
			// Si es menos de 1 mes, establecer a 1 para evitar división por cero
			if($monthsDifference < 1){
				$monthsDifference = 1;
			}
			
			// Calcular el monto de ahorro mensual
