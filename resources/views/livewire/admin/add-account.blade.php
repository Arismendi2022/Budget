						...
						<div id="accounts-container-{{ $group->type }}" class="accounts-container">
							@foreach($group->accounts as $account)
								<a id="ember{{ $account->id }}" draggable="true" class="nav-account-row {{ isSelected('admin.account-detail', ['id' => $account->id]) }}"
									...
									<div class="nav-account-icons nav-account-icons-left js-nav-account-icons-left" title="Edit Account" wire:click="openEditAccountModal({{ $account->id }})"
                        ...
                        <div class="account-widget-loan-category-radio-description-subtitle">Pair with a brand new category</div>
								...
								@elseif ($selectedOption === 'new')
									<div class="y-form-field field-with-error ">
										...
										<input id="nickname" wire:model="nickname" class="ember-text-field ember-view y-input account-widget-loan-category-new-subcategory" autocomplete="nope"
					...
					@elseif($currentSection === 5)
						<div class="account-widget-step account-widget-success-screen">
							...
							<div class="account-widget-body">
								...
								<h3>Success!</h3>
								<p>Add transactions on the web or in our mobile apps. You can also download a transaction file from your institution and use <a href="#" onclick="return false;"
										target="_blank" rel="noopener noreferrer">File-Based Import</a>. </p>
							</div>
							<div class="account-widget-footer">
...
@push('scripts')
	<script>
		...
		/* Funcion para Drag & Drop */
		document.addEventListener('DOMContentLoaded', function () {
			// Selecciona todos los contenedores de cuentas
			$('.accounts-container').sortable({
				...
				stop: function (event, ui) {
					...
					// Captura el orden de las cuentas después de soltar
