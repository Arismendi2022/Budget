...
@section('content')
                  ...
                  Edit Account
												...
												@if($account->account_group !== 'Loans')
														<button class="ynab-button primary accounts-header-reconcile tooltip-relative-container" aria-describedby="reconcile-button-tooltip" type="button">
																Reconcile
																<span role="tooltip" id="reconcile-button-tooltip" class="tooltip-content tooltip-below tooltip-right ">
        ...
        <span class="user-data">
          <span class="user-data currency {{ $account->balance > 0 ? 'positive' : 'negative' }}">{{ (in_array($account->data_type, ['CreditCard', 'LineOfCredit']) || $account->balance < 0) ? '−' : '' }}
		          <bdi>$</bdi>{{ number_format(abs($account->balance), 2, ',', '.') }}</span>
        </span>
												<div class="accounts-header-label">
														...
														Cleared Balance
														<span role="tooltip" id="add-transaction" class="tooltip-content tooltip-below tooltip-left ">
        ...
        <span class="user-data">
          <span class="user-data currency zero"><bdi>$</bdi>0,00</span>
												...
												<div class="accounts-header-label">
														...
														Uncleared Balance
														<span role="tooltip" id="add-transaction" class="tooltip-content tooltip-below tooltip-left ">
             The total of all uncleared transactions. This includes transactions you've manually entered that your bank does not know about yet, as well as only those pending transactions from your bank that you've chosen to Enter Now.
            </span>
												</div>
										</div>
										<i>
        ...
        <span class="user-data">
          <span class="user-data currency {{ $account->balance > 0 ? 'positive' : 'negative' }}">{{ ($account->account_group !== 'Loans' && (in_array($account->data_type, ['CreditCard', 'LineOfCredit']) ||
          ...
          number_format(abs($account->balance), 2, ',', '.') }}</span>
												...
												<div class="accounts-header-label">
														...
														Working Balance
														<span role="tooltip" id="add-transaction" class="tooltip-content tooltip-below tooltip-left ">
             The sum of your cleared and uncleared transactions. This represents the money you have available to spend (or owe, in the case of credit cards and loan accounts).
            </span>
												</div>
										</div>
            ...
            <div class="shortcut-tooltip">
             ...
             Import transactions from a bank file.
												...
												<!---->
												@if(in_array($account->data_type, ['CreditCard', 'LineOfCredit']))
														<button class="ghost-button primary type-body js-record-payment" aria-describedby="ember86" type="button">
																<svg class="ynab-new-icon" width="16" height="16">
																		<!---->
          ...
          <button disabled="" class="ghost-button primary type-body undo-button" aria-describedby="ember236" type="button">
            <svg class="ynab-new-icon" width="12" height="12">
              ...
              <use href="#icon_sprite_undo">
		              <symbol xmlns="http://www.w3.org/2000/svg" id="icon_sprite_undo" fill="none" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd"
          ...
          <button disabled="" class="ghost-button primary type-body redo-button" type="button">
            <svg class="ynab-new-icon" width="12" height="12">
              ...
              <use href="#icon_sprite_redo">
		              <symbol xmlns="http://www.w3.org/2000/svg" id="icon_sprite_redo" fill="none" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd"
            ...
            Redo
										...
										<div class="accounts-toolbar-right">
												<button class="ghost-button primary is-dropdown type-body accounts-toolbar-view-options" aria-describedby="ember238" type="button">
														View
												...
												<div id="ember239" class="transaction-search js-transaction-search ">
														<div class="transaction-search-inner">
																...
																<input id="ember240" class="ember-text-field ember-view transaction-search-input" placeholder="Search {{$account->nickname}}"
																       ...
																       type="text">
																<!---->
																{{-- <button class="transaction-search-cancel-icon js-transaction-search-cancel-icon" type="button">
																		<svg class="ynab-new-icon" width="12" height="12">
																				<!---->
										...
										<div class="loan-account-header-balances">
												<div class="loan-account-header-element">
														<div class="loan-account-header-element-amount loan-account-header-element-balance">
																<div id="ember390" class="animate-number  ">
																		<span class="user-data currency positive"><bdi>$</bdi>{{ number_format(abs($account->balance), 2, ',', '.') }}</span>
																</div>
														</div>
												...
												<div class="loan-account-header-element">
														<div class="loan-account-header-element-amount loan-account-header-element-interest-rate">
																<span>{{ fmod($account->interest, 1) == 0 ? intval($account->interest) : number_format($account->interest, 1, ',', '') }}<bdi>%</bdi></span>
														</div>
												...
												<div class="loan-account-header-element">
														<div class="loan-account-header-element-amount loan-account-header-element-min-payment-amount">
																<span class="user-data currency positive"><bdi>$</bdi>{{ number_format(abs($account->payment), 2, ',', '.') }}</span>
														</div>
												...
												<div class="loan-account-header-element">
														<div class="loan-account-header-element-amount loan-account-header-element-payoff-date">
																<strong>{{ $account->created_at->format('M Y') }}</strong>
														</div>
...
@push('scripts')
		<script>
		// Selecciona el input y el contenedor
		const input = document.getElementById('ember240');
		const container = document.getElementById('ember239');
